  ...
  return (
    ...
    >
      <div className="grid gap-5 lg:grid-cols-[minmax(0,1.28fr)_minmax(0,0.92fr)] lg:items-stretch">
        <article
          className={`surface-card group relative overflow-hidden rounded-[1.65rem] bg-[#0d131d]/88 p-6 transition duration-300 ease-out hover:-translate-y-1 ${featuredTone.glow} md:p-7 lg:min-h-[32rem]`}
        >
          ...
          <div className="relative flex h-full flex-col">
            ...
            <div className="mt-6 flex flex-wrap gap-2">
              {featuredRepository.stack.map((tag) => (
                <span
                  key={tag}
                  className="surface-chip rounded-full bg-white/[0.04] px-2.5 py-1 text-[10px] font-semibold uppercase tracking-[0.12em] text-text/72"
                >
                  {tag}
                </span>
              ))}
            ...
            <div className="mt-8 rounded-2xl bg-black/18 p-4 backdrop-blur-sm shadow-[inset_0_1px_0_rgba(255,255,255,0.03)]">
        ...
        <div className="grid gap-5">
          {secondaryRepositories.map((repo) => {
            ...
            return (
              <article
                key={repo.name}
                className={`surface-card group relative overflow-hidden rounded-[1.4rem] bg-[#0d131d]/84 p-5 transition duration-300 ease-out hover:-translate-y-1 ${tone.glow} md:p-6`}
              >
                ...
                <div className="relative">
                  ...
                  <div className="mt-4 flex flex-wrap gap-2">
                    {repo.stack.slice(0, 4).map((tag) => (
                      <span
                        key={tag}
                        className="surface-chip rounded-full bg-white/[0.035] px-2.5 py-1 text-[10px] font-semibold uppercase tracking-[0.11em] text-text/68"
                      >
                        {tag}
                      </span>
                    ))}
