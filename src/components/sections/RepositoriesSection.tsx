...
const repositoryToneStyles = {
  automation: {
    badge: "text-[#78a9ff] bg-[#78a9ff]/14",
    edge: "from-[#78a9ff]/70 via-[#3d66ff]/35 to-transparent",
    glow: "hover:shadow-[0_20px_48px_rgba(0,0,0,0.34),0_0_24px_rgba(120,169,255,0.11)]",
  ...
  frontend: {
    badge: "text-[#9ca7ff] bg-[#9ca7ff]/14",
    edge: "from-[#9ca7ff]/70 via-[#6f7cff]/30 to-transparent",
    glow: "hover:shadow-[0_20px_48px_rgba(0,0,0,0.34),0_0_24px_rgba(156,167,255,0.11)]",
  ...
  product: {
    badge: "text-[#88d7ff] bg-[#88d7ff]/14",
    edge: "from-[#88d7ff]/70 via-[#8d7bff]/28 to-transparent",
    glow: "hover:shadow-[0_20px_48px_rgba(0,0,0,0.34),0_0_24px_rgba(136,215,255,0.1)]",
...
export function RepositoriesSection(): JSX.Element {
  ...
  const primaryButtonClass =
    "inline-flex items-center gap-2 rounded-full bg-white px-3.5 py-2.5 text-[13px] font-semibold text-[#0a1018] transition duration-200 hover:bg-white/92 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent focus-visible:ring-offset-2 focus-visible:ring-offset-bg sm:px-4 sm:text-sm";
  const secondaryButtonClass =
    "inline-flex items-center gap-2 rounded-full bg-white/[0.055] px-3.5 py-2.5 text-[13px] font-semibold text-text/82 transition duration-200 hover:bg-white/[0.09] hover:text-text focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent-2 focus-visible:ring-offset-2 focus-visible:ring-offset-bg sm:px-4 sm:text-sm";
          ...
          <div className="relative flex h-full flex-col">
            <div className="flex flex-wrap items-start justify-between gap-4">
              <div className="space-y-3">
                <span className={`surface-chip inline-flex rounded-full px-3 py-1 font-mono text-[10px] uppercase tracking-[0.18em] ${featuredTone.badge}`}>
                  {featuredRepository.label}
                </span>
                <div>
                  <p className="font-mono text-[11px] uppercase tracking-[0.16em] text-text/36">Featured codebase</p>
                  <h3 className="mt-2 font-heading text-[1.45rem] tracking-[-0.04em] text-text sm:text-[2rem]">
            ...
            <p className="mt-4 max-w-[52ch] text-[0.95rem] leading-6 text-text/88 sm:mt-6 sm:text-[1.02rem] sm:leading-7">
            ...
            <div className="mt-5 flex flex-wrap gap-2 sm:mt-6">
              {featuredRepository.stack.map((tag) => (
                <span
                  key={tag}
                  className="surface-chip rounded-full bg-white/[0.04] px-2.5 py-1 text-[9px] font-semibold uppercase tracking-[0.12em] text-text/72 sm:text-[10px]"
                >
                  {tag}
                </span>
              ))}
            ...
            <div className="mt-6 rounded-2xl bg-black/18 p-3.5 backdrop-blur-sm shadow-[inset_0_1px_0_rgba(255,255,255,0.03)] sm:mt-8 sm:p-4">
                ...
                <div className="relative flex h-full flex-col">
                  <div className="flex items-start justify-between gap-4">
                    <div className="min-w-0">
                      ...
                      <h3 className="mt-3 text-[1.08rem] font-semibold tracking-[-0.03em] text-text sm:mt-4 sm:text-[1.35rem]">
                        {repo.name}
                      </h3>
                    </div>
                  </div>

                  <p className="mt-2.5 max-w-[40ch] text-[13px] leading-6 text-text/80 sm:mt-3 sm:text-sm">{repo.description}</p>

                  <div className="mt-4 flex flex-wrap gap-2">
                    {repo.stack.slice(0, 4).map((tag) => (
                      <span key={tag} className="surface-chip rounded-full bg-white/[0.035] px-2.5 py-1 text-[9px] font-semibold uppercase tracking-[0.11em] text-text/68 sm:text-[10px]">
                        {tag}
                      </span>
                    ))}
                  </div>
